# -*- coding: utf-8 -*-
"""Motif analysis: knee finding on Pareto fronts and structural feature extraction."""
...
# ------- bits -> rule matrix (supports fallback) -------
_HAS_EVAL = False
try:
...
# ---------- Knee finders ----------
def knee_second_diff(xs, ys, logy=True):
...
def knee_lcurve(xs, ys, logxy=True):
    xs = np.asarray(xs, float); ys = np.asarray(ys, float)
    if logxy:
...
def _odd_girth(G:np.ndarray)->float:
    # heuristic upper-bound; return inf if none
    k = G.shape[0]
    INF = 1e9
    best = INF
    for s in range(k):
    ...
    for u in range(k):
        for v in range(u+1,k):
            ...
            for x in range(k):
                ...
                if not G[u,x]: continue
                for y in range(k):
                    if y<=x or y==u or y==v: continue
                    if G[v,y] and G[x,y]:
                        c4 += 1
            oddchord = 0
            for w in range(k):
...
def _y_metric(row:dict)->float:
    for key in ("sum_lambda_powers","Z_exact","y","objective"):
        val = row.get(key, "")
        if val != "":
            try:
                return float(val)
...
def analyze_fronts_for_knees(csv_paths: List[str],
                             ...
                             style: str = "default",
                             logy: bool = True):
    """Main entry: group by (n,k), build growth curve per |R|, locate knee, extract features.
    Returns: (examples_csv, summary_csv, global_csv, fig_paths)
    """
    apply_style(style)
    ...
    for r in rows:
        if not _is_front_row(r):
            continue
        n = to_int(r.get("n"))
        k = to_int(r.get("k"))
        if n is None or k is None:
            fn = r.get("__file__", "")
            nk = parse_nk_from_filename(fn) if fn else None
            if nk is None:
                # attempt parse from run_tag if any
                tag = r.get("run_tag","" )
                nk = parse_nk_from_filename(tag) if tag else None
            if nk is None:
                # skip if still unknown
                continue
            n,k = nk
        by_nk.setdefault((n,k), []).append(r)
    ...
    for (n,k), rs in sorted(by_nk.items()):
        # build buckets by rule_count with best y
        buckets = {}
        for r in rs:
        ...
        if idx_knee is None:
            # still record minimal info for completeness, but skip feature deltas
            continue
        ...
        for pos in ("pre","knee","post"):
            ...
            # GA outputs may store bits under different keys
            if (not bits_s) and ("bits" in r):
                bits_s = str(r.get("bits","" )).strip()
            if not bits_s:
                for f in ["deg_max","deg_mean","deg_std","diag_cnt","selfloop_rich",
                          "tri","c4","c5","odd_girth","near_bip_chord","clustering",
                          "kcore","lambda1","lambda2","gap","lap_algebraic","star_core",
    ...
    # summary & global
    total_knee = sum(1 for r in examples if isinstance(r.get("knee_bits",""), str))
    ...
    features = ["selfloop_rich","star_core","near_bip_chord","tri","c5","gap_pos"]
    summ_rows=[]
    for feat in features:
        kn = knee_feat_counter.get(feat,0); pr=pre_feat_counter.get(feat,0); po=post_feat_counter.get(feat,0)
        kn_p = kn / max(1,total_knee); pr_p = pr / max(1,total_pre); po_p = po / max(1,total_post)
        or_pre  = ((kn+0.5)/(total_knee+1)) / (((pr+0.5)/(total_pre+1)) if total_pre>0 else 1.0)
        or_post = ((kn+0.5)/(total_knee+1)) / (((po+0.5)/(total_post+1)) if total_post>0 else 1.0)
        summ_rows.append(dict(feature=feat,
                              knee_count=kn, knee_total=total_knee, knee_ratio=kn_p,
    ...
    if summ_rows:
        ...
        with open(sum_path, "w", newline="", encoding="utf-8") as f:
            w = _csv.DictWriter(f, fieldnames=list(summ_rows[0].keys()))
    ...
    if glob:
        ...
        with open(glob_path, "w", newline="", encoding="utf-8") as f:
            w = _csv.DictWriter(f, fieldnames=list(glob[0].keys()))
    ...
    try:
        ...
        if labels:
            knee_ratio = [r["knee_ratio"] for r in summ_rows]
            pre_ratio  = [r["pre_ratio"]  for r in summ_rows]
            post_ratio = [r["post_ratio"] for r in summ_rows]
            ...
            plt.ylabel("ratio"); plt.title("Knee-driving motif prevalence") ; plt.legend(frameon=False)
            plt.tight_layout()
        ...
        for (n,k), rs in sorted(by_nk.items()):
            buckets={}
            for r in rs:
                rc = to_int(r.get("rule_count")); y=_y_metric(r)
                if rc is None or not np.isfinite(y): continue
                cur=buckets.get(rc)
                if (cur is None) or (y>cur["y"]): buckets[rc]=dict(row=r,y=float(y))
            if not buckets: continue
            xs=sorted(buckets.keys()); ys=np.array([buckets[x]["y"] for x in xs],float)
            idx=robust_knee(xs,ys,logy=True)
            if idx is None: continue
            xs3=[]; ys3=[]
            for j in [idx-1, idx, idx+1]:
                if 0<=j<len(xs):
                    xs3.append(xs[j]); ys3.append(ys[j])
            if len(xs3)>=2:
                plt.plot(xs3, ys3, marker="o", alpha=0.7, label=f"n{n}k{k}")
        plt.yscale("log"); plt.xlabel("|R|"); plt.ylabel("trace estimate / Z_exact")
        plt.title("Knee points (three-point link)")
        plt.legend(frameon=False, ncol=2); plt.tight_layout()
        plt.savefig(out_fig_dir / "knee_gap_link.png", dpi=200); plt.close()
    except Exception:
        pass

    return str(ex_path), str(sum_path), str(glob_path), [str(out_fig_dir/"knee_motif_bar.png"), str(out_fig_dir/"knee_gap_link.png")]
