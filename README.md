...
| 边界模式 | `--boundary {torus,open}` | `boundary` | `torus` 为循环边界；`open` 仅走精确计数路径（禁用谱估计，当前仅 CPU，规模大时行数爆炸需谨慎，适合小规模 $(n,k)$）。 |
...
> `--boundary open` 仅支持精确计数路径（自动禁用谱估计），当前实现只跑在 CPU 上；行枚举随规模增长极快，建议仅在小型 $(n,k)$ 上启用。
...
n: 5
k: 4
boundary: torus          # 也支持 open（仅精确计数、禁用谱估计，当前仅 CPU，规模大需谨慎）
sym_mode: perm
