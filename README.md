...
| 边界模式 | `--boundary {torus,open}` | `boundary` | `torus` 为循环边界；`open` 为非循环边界，当前仅走精确计数（CPU），规模过大时会非常慢。 |
...
> **边界提示**：`--boundary open` 下 GA 会强制关闭谱估计/快速评估，仅使用精确计数（CPU），行数随规模指数增长，建议只在小尺度网格上启用。
...
n: 5
k: 4
boundary: torus          # 也支持 open（仅精确计数 / CPU 路径，行数暴增时请谨慎）
sym_mode: perm
